      ...
      <div className="flex flex-col flex-1 overflow-y-auto scrollbar-none px-6 pt-6 pb-4">
        <div className="flex justify-between items-start mb-5 flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-muted mt-1">Pick up where you left off</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <Bell className="w-7 h-7 text-emerald-500 fill-emerald-500/10" />
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 bg-dashboard-card border border-neutral-800 rounded-xl px-4 py-3 mb-5 flex-shrink-0">
          ...
          <input
            ...
            className="bg-transparent text-sm w-full outline-none text-neutral-300 placeholder-neutral-500 border-none p-0 focus:ring-0"
          />
        </div>
        ...
        <div className="grid grid-cols-3 gap-y-5 gap-x-2 mb-6 flex-shrink-0">
          {categories.map(({ label, icon: Icon, bg, border, color }) => (
            <div key={label} className="flex flex-col items-center text-center gap-2 cursor-pointer group">
              <div className={`w-12 h-12 rounded-full ${bg} border ${border} flex items-center justify-center group-hover:scale-105 transition`}>
        ...
        <div className="bg-gradient-to-r from-emerald-900/40 to-cyan-950/40 border border-emerald-800/30 rounded-2xl p-4 mb-6 flex items-center justify-between flex-shrink-0 shadow-lg">
          <div className="max-w-[65%]">
            <h3 className="text-sm font-bold text-neutral-200">Ready to level up?</h3>
            <p className="text-[13px] text-neutral-400 mt-0.5 leading-tight">Register for new cohort tracks today.</p>
          </div>
          <Link
            href="/register"
            className="bg-emerald-600 hover:bg-emerald-500 active:scale-95 text-white text-[14px] font-bold px-3.5 py-2 rounded-xl shadow-md shadow-emerald-600/20 transition-all flex items-center gap-1"
          >
            <span>Register</span>
            ...
            {courses.map(({ name, rating, icon: Icon, bg, border, hover, color }) => (
              <div key={name} className="flex-shrink-0 w-32 cursor-pointer group">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  <span className="text-[14px] text-neutral-400 font-bold">{rating}</span>
