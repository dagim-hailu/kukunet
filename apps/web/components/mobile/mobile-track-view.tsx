    ...
    <div className="flex flex-col min-h-full bg-[var(--bg)] text-[var(--text)] transition-colors duration-300">
      <div className="flex flex-col flex-1 overflow-y-auto scrollbar-none px-6 pt-6 pb-4">
        ...
        {activeTab === 'core' ? (
          <>
            <div className="grid grid-cols-2 gap-4 mb-6 flex-shrink-0">
              <button
                ...
                className="bg-emerald-950/30 border border-emerald-900/30 rounded-2xl p-4 flex flex-col justify-between h-32 cursor-pointer hover:bg-emerald-950/40 transition text-left"
              >
                ...
                <div>
                  <h3 className="text-xs font-bold text-emerald-400">Quiz</h3>
                  <p className="text-[10px] text-emerald-600 font-medium mt-0.5">Test your skills</p>
                </div>
              </button>
              <button
                type="button"
                onClick={() => alert('Opening Summary notes...')}
                className="bg-cyan-950/20 border border-cyan-900/20 rounded-2xl p-4 flex flex-col justify-between h-32 cursor-pointer hover:bg-cyan-950/30 transition text-left"
              >
                <div className="w-7 h-7 rounded-lg bg-cyan-400 flex items-center justify-center shadow-lg shadow-cyan-400/20">
                  <FileText className="w-4 h-4 text-neutral-900" />
                </div>
                <div>
                  <h3 className="text-xs font-bold text-cyan-400">Summarize</h3>
                  <p className="text-[10px] text-cyan-600 font-medium mt-0.5">Review notes</p>
                </div>
              </button>
            </div>

            <div className="mb-3 flex-shrink-0">
              <h2 className="text-sm font-semibold tracking-wide">Continue learning</h2>
              <p className="text-[11px] text-muted mt-0.5">Your curated learning path</p>
            </div>
            ...
            <div className="space-y-2.5">
              {courses.map((course) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0 ${course.iconBg}`}>
                      <Icon className={`w-4 h-4 ${course.iconColor}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-xs font-semibold truncate">{course.name}</h4>
                      <p className="text-[10px] text-muted mt-0.5">{course.detail}</p>
                    </div>
                    <button
                      type="button"
                      disabled={course.disabled}
                      onClick={() => !course.disabled && alert(`Starting ${course.name}...`)}
                      className={`text-[10px] font-bold px-3 py-1.5 rounded-full shadow-md transition cursor-pointer border-none ${course.actionClass} ${
          ...
          <div className="flex-1 flex items-center justify-center text-center p-6 text-neutral-500 text-xs">
